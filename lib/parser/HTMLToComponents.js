class HTMLToComponents {
    constructor(configOptions) {
        ...
        this.nodeHandlers = {
            ...
            'OL': (node) => this.handleOrderedListComponent.bind(this)(node),
            'UL': (node) => this.handleUnorderedListComponent.bind(this)(node),
            'LI': (node) => this.handleListItemComponent.bind(this)(node)
        };
    }
    resetParser() {
        this.parsed = [];
    ...
    handleOrderedListComponent(htmlNode) {
        const content = [];
        htmlNode.childNodes.forEach(child => {
            const childComponent = this.nodeHandlers[child.nodeName](child);
            if (childComponent)
    ...
    handleUnorderedListComponent(htmlNode) {
        const content = [];
        htmlNode.childNodes.forEach(child => {
            const childComponent = this.nodeHandlers[child.nodeName](child);
            if (childComponent)
    ...
    handleListItemComponent(htmlNode) {
        const content = [];
        htmlNode.childNodes.forEach(child => {
            const childComponent = this.nodeHandlers[child.nodeName](child);
            if (childComponent)
